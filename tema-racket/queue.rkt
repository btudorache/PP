...
(define (queue-empty? q)
  (= 0 (+ (queue-size-l q) (queue-size-r q))))
...
(define (rotate-queue q)
  (struct-copy queue q
               [left (rotate (queue-left q) (queue-right q) empty-stream)]
               [right '()]
               [size-l (+ (queue-size-l q) (queue-size-r q))]
               [size-r 0]))
...
(define (enqueue x q)
  (let ((added-queue (struct-copy queue q
                                  [right (cons x (queue-right q))]
                                  [size-r (add1 (queue-size-r q))])))
    (if (= 1 (- (queue-size-r added-queue) (queue-size-l added-queue)))
      (rotate-queue added-queue)
      added-queue)))
